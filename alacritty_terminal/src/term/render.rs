...
impl<'a, T, C> RenderableContent<'a, T, C> {
    pub fn new(term: &'a Term<T>, config: &'a Config<C>, show_cursor: bool, grep_search: bool, search_query: String, grep_after: usize) -> Self {
        // Cursor position.
        ...
        Self {
            display_iter: term.grid.display_iter(grep_search, search_query.clone(), grep_after),
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            if self.cursor_point == self.display_iter.point() {
                ...
                let mut cell = RenderableCell::new(self, cell, self.grep_search);
...
impl RenderableCell {
    fn new<'a, T, C>(content: &mut RenderableContent<'a, T, C>, cell: Indexed<&Cell>, grep_search: bool) -> Self {
        ...
        // println!("Going to take {:?} and advnace to {:?}", point, grid.visible_to_buffer(point));
        if is_selected {
        ...
        // } else if content.search.advance(grid.visible_to_buffer(point)) {
            // TODO: Important - fix this so that it scrolls right to the text
