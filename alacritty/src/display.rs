...
impl Display {
    ...
    pub fn draw<T>(
        ...
        let mut new_line: Vec<alacritty_terminal::term::render::RenderableCell> = Vec::new();

        let mut cursor: Option<RenderableCursor> = None;

        
        let search_query: String  = match search_state.regex() {
            Some(sq) => {
                sq.clone()
            }, 
            None => {
        ...
        let search_query_clone_regex = search_query.clone();

        let mut content = terminal.renderable_content(config, !cursor_hidden,  search_state.grep_mode(), search_query, config.ui_config.grep_after);

        let re = regex::Regex::new(&search_query_clone).unwrap();
        ...
        if (false) {
            ...
            let popover_rect =
                RenderRect::new(0.0, 0.0, size_info.width(), size_info.height(), config.colors.normal.black, 0.95);
